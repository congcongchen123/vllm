...
from vllm.distributed import (ensure_model_parallel_initialized,
                              ensure_context_parallel_initialized,
...
def init_worker_distributed_environment(
    ...
    ensure_context_parallel_initialized(parallel_config.tensor_parallel_size,
                                       parallel_config.pipeline_parallel_size,
                                       parallel_config.context_parallel_size)
...
def raise_if_cache_size_invalid(num_gpu_blocks, block_size,
                                max_model_len, context_parallel_size) -> None:
    if num_gpu_blocks <= 0:
        raise ValueError("No available memory for the cache blocks. "
                         "Try increasing `gpu_memory_utilization` when "
                         "initializing the engine.")
    max_seq_len = block_size * num_gpu_blocks
    if max_model_len > max_seq_len * context_parallel_size:
        raise ValueError(
            f"The model's max seq len ({max_model_len}) "
            "is larger than the maximum number of tokens that can be "
            f"stored in KV cache ({max_seq_len * context_parallel_size}). Try increasing "
